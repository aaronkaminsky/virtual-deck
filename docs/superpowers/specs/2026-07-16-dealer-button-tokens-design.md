...
| Re-anchoring | An anchored token is itself a drag source (`useDraggable`, same as tray/canvas tokens) — dragging it from Player A's name row directly onto Player B's row re-anchors it in one motion, matching the physical "pass the button" ritual. Dragging it to the tray un-anchors; dragging it to the felt frees it onto the canvas |
...
- **New: anchored token rendering in `HandZone` and `OpponentHand`.** Both components already render a name row (`flex items-center gap-2 px-{1,4} mb-1` containing the presence dot, name, and `chipsEnabled && <ChipBadge>`). A token whose `placement.kind === 'player'` and `placement.playerId` matches that zone's `playerId` renders in that same row, positioned right after the name (before the chip badge), as a **draggable** `TokenDisc` (`size="sm"`) — same `useDraggable` id/data contract as tray and canvas tokens (`` `token-${tokenId}` ``, `{ type: 'token', tokenId }`), so it can be re-anchored directly. `BoardView` derives `anchoredTokenIds(playerId): TokenId[]` once per render and passes the filtered list down, so each hand component stays agnostic of the full token list.
...
- **Unit (Vitest, TDD Wave 0 RED scaffolds first):**
  ...
  - Client: `HandZone`/`OpponentHand` render an anchored token's `TokenDisc` in the name row when `placement.kind === 'player'` matches that zone's `playerId`, and render nothing when it doesn't; the rendered disc carries the same draggable id/data contract as tray/canvas tokens.
- **E2e (Playwright, two `BrowserContext`s):**
  ...
  - Drag the same token directly from Player 2's name row to Player 1's own hand row (one drag, no tray stop) → anchor moves; both clients update.
