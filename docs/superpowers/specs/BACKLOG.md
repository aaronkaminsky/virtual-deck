...
| 1036 | Review usability of Top/Bottom/Random dialog when dropping on a pile. It is nice to have the capability, but the most common case by far is Top, and adding an extra click for that action is annoying. We should come up with some changes to this UX so that you can optionally add to bottom or random, but the default is to immediately drop on the top. |
