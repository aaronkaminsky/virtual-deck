...
| 1009 | When you make your hand visible, the order should match, so if the user used the sort feature, the state might not match, so maybe when you make your hand visible that should trigger an event to sync the client hand order to the server? |
