...
- **Spread:** at rest, only `ChipStack` (coin stack + `chipsInSpread`) is visible. On hover/focus: the "Move to pot" button (full-amount, one-click) and a kebab trigger appear.
...
### 3. Kebab → Popover secondary actions
...
- `src/components/ChipStack.tsx` — three coins, fixed 5px gaps, absolute positioning (replaces the negative-margin flex approach).
- `src/components/HandZone.tsx` — wrap the existing bet input/button in `zone-controls`; add a kebab + `Popover` with the "To pot" action; remove the standalone "To pot" button from the always-visible row.
- `src/components/SpreadZone.tsx` — replace the always-visible amount input with a kebab + `Popover` (defaulting to full `chipsInSpread`, two buttons); wrap "Move to pot" in `zone-controls`.
- `src/components/PotZone.tsx` — add a `zone-hover` wrapper; wrap "Take all" in `zone-controls`; replace the always-visible amount input + Hand/Bet buttons with a kebab + `Popover` (defaulting to full `pot`, two buttons: "Hand"/"Bet").
